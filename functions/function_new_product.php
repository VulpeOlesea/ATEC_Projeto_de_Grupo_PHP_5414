...
function function_new_product($prod_name, $prod_price, $prod_description, $prod_category, $prod_image){
	include 'connections/config.php';
	$img = $_FILES["prod_image"]["name"];
	$temp = explode(".", $_FILES["prod_image"]["name"]);
	$new_img = round(microtime(true)) . '.' . end($temp);

	move_uploaded_file($_FILES["prod_image"]["tmp_name"], 'img/prod/'.$new_img);
	...
	mysqli_query($conn,"INSERT INTO products (prod_name, prod_price, prod_description, prod_category, prod_image) 
		VALUES ('$prod_name','$prod_price','$prod_description','$prod_category','$new_img')");
